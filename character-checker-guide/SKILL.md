...
description: |
  AI Character Checker のエントリポイント＆ルーティングスキル。
  「どのスキルを使えばいい？」「使い方を教えて」「どこから始めればいい？」
  「案内して」「ガイド」「何ができる？」と聞かれたとき、または
  ユーザーの目的・使用スキルが不明確なときに最初に使用する。
  渡された情報（キャラクター設定・AIプロンプト・蒸留結果など）の種類と
  ユーザーの目的をもとに、最適なスキルへ自動ルーティングするか、
  目的のヒアリングを行う。スキル名を直接指定された場合は、このスキルは使用しない。
---
