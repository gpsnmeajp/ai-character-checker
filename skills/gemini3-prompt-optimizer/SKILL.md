...
description: >
  汎用的なLLMプロンプトをGemini 3（3.0/3.1）で最高性能が出る形式に変換・最適化するスキル。
  ユーザーが「プロンプトをGemini用に書き直して」「Gemini 3向けに最適化して」「このプロンプトをGeminiで動くようにして」
  「プロンプト変換」「Gemini移行」などと言った場合に使用する。
  ChatGPT/Claude/GPT-4/Llama等の他モデル向けプロンプトの移行、既存プロンプトの改善、
  新規Gemini 3プロンプトの作成にも対応する。プロンプトエンジニアリング、プロンプト改善、
  Gemini最適化に関する依頼では積極的にこのスキルを使うこと。
---
