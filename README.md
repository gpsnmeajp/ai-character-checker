...
![6軸診断イメージ](img/header.jpg)

キャラクターを多角的に分析・診断・自動改善を試行するスキル集です。  
加えて、汎用的なプロンプト全般に適用できる可能性の高いプロンプト強化スキルも提供しています。  
...
**診断だけではなく、安全な修正、プロンプトの強化、新規キャラクターの安定設計、キャラクターの物語的変換まで**、20のスキルで一貫して扱えます。  
...
![AI Character Checker](img/AI%20Character%20Checker.jpg)
...
- 内部に18スキル分のリファレンスを同梱し、状況に応じて自動で参照
...
![](img/character-type-checker.jpg)
...
![](img/ai-character-stability.jpg)
...
![](img/ai-self-description-analyzer.jpg)
...
![](img/ai-user-conflict-predictor.jpg)
...
![](img/character-role-analyzer.jpg)
...
![](img/roleplay-burden-scorer.jpg)
...
![](img/character-cultural-value-checker.jpg)
...
![](img/romantization-chain-detector.jpg)
...
![](img/ai-fault-mode-deflector.jpg)
...
![](img/character-prompt-fortifier.jpg)

---

#### [`character-prompt-fortifier-for-gemini3`](skills/character-prompt-fortifier-for-gemini3/SKILL.md)

既存のAIキャラクタープロンプトの崩壊耐性をGemini 3向けに強化するスキル。  
**安全を保証するものではありません**

- `character-prompt-fortifier` のGemini 3特化版
- 12の強化技法 + Gemini 3固有の防御設計（迎合抑止・ハルシネーション防御・Split-Step Verification）
- `gemini3-prompt-optimizer` の最適化知識を統合
- Gemini 3のアンチパターン（リスト過多・トリガーワード依存等）を回避

---

### 特殊改善スキル

#### [`general-prompt-fortifier`](skills/general-prompt-fortifier/SKILL.md)

汎用プロンプト（分類器・レビューツール・分析器・エージェントなど）の崩壊耐性を強化するスキル。  
`character-prompt-fortifier` からキャラクター要素を除去した汎用版。

- `character-prompt-fortifier` と共通の理論的基盤（LLMは物語性生成器である）
- 11の強化技法でクロスモデル互換性を向上
- プロンプトを一人称自述形式に変換し、制約を内在化
- SLM（小型モデル）から推論モデルまで幅広く対応
- キャラクター要素（口調・世界観・関係性・連鎖耐性）は含まない

![](img/general-prompt-fortifier.jpg)

---

#### [`general-prompt-fortifier-for-gemini3`](skills/general-prompt-fortifier-for-gemini3/SKILL.md)

汎用プロンプトの崩壊耐性をGemini 3向けに強化するスキル。  
`general-prompt-fortifier` のGemini 3特化版。

- 11の強化技法 + Gemini 3固有の防御設計
- `gemini3-prompt-optimizer` の最適化知識を統合
- キャラクター要素は含まない汎用版

---

#### [`gemini3-prompt-optimizer`](skills/gemini3-prompt-optimizer/SKILL.md)

汎用プロンプトをGemini 3最適化形式に変換するスキル。

- アンチパターン回避・テンプレート・変換ルールを参照ファイルに外部化
- Gemini 3の特性（thinkingレベル、迅速応答性等）に最適化
- 崩壊耐性強化ではなく、フォーマット変換に特化
...
![](img/stable-character-creator.jpg)
...
![](img/character-burnout-converter.jpg)
...
危険判定される子、主役判定される子は、こっちで動かすとすごくよく動くと思います。  
[参加型AI物語生成ツール Narrative Conversation](https://github.com/gpsnmeajp/NarrativeConversation)
