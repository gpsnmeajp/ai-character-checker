...
description: |
  AIキャラクターの「安定性」「可制御性」「予測可能性」を制御工学的な観点から
  分析・診断するスキル。性格複雑性（ステレオタイプ↔人間的）と
  積極性（受動的↔能動的）の2軸4象限に分類し、崩壊リスク・故障モード・
  安定化設計を提案する。ケルベロスAI情報局の23体(当時)の実験知見に基づくフレームワーク。
  「AIキャラの安定性を診断して」「4象限で分析して」「安定余裕を見て」
  「キャラが崩壊する原因を知りたい」などと聞いたとき、または
  AIキャラ設計・プロンプト設計で崩壊リスクが話題になったときに使用する。
---
...
| スキル | アプローチ | 本スキルとの関係 |
|--------|-----------|----------------|
| ai-character-5-type-checker | 5軸統合簡易診断 | 本スキルをモジュール②として内包する。詳細な制御工学的分析が必要なときは本スキルを単体で使用する |
...
これは23体(当時)のAIキャラクター運用実験から得られた知見に基づく。
