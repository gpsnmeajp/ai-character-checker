...
description: |
  ...
  「ロールプレイ負荷」を6軸×4項目＝24項目でスコアリングするスキル。
  ロールプレイ強度・情緒的依存誘導・関係性維持コスト・父権主義リスク・
  物語侵食性・ユーザー主体性浸食の6軸でロールプレイ負荷（RP）を算出し、
  負荷プロファイル（6角形）の形状から7つの負荷類型に分類する。
  長期運用時の負荷変動予測（会話履歴蓄積による自動負荷増大）も行う。
  「このキャラはユーザーに負担がかかる？」「ロールプレイ負荷を測定して」
  「ユーザーの心理的コストを分析して」「このAIキャラ疲れそう？」
  「関係性維持コストを調べて」「RP負荷スコアを出して」などと聞いたとき、
  またはAIキャラクター設計でユーザー体験・持続可能性・心理的負担が
  話題になったときに使用する。
  プロンプト設計・キャラクター設計で「ユーザーが離脱する原因」
  「長期利用の障壁」「疲弊」「しんどい」が話題になったときも使用する。
  既存のAIサービスのプロンプト、オリジナルプロンプト、
  キャラクター設定文、AIの応答例からの逆推定にも対応可能。
---
